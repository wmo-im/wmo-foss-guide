...
Much of FOSS's development and maintenance is undertaken by individual contributors, for example, developers, researchers
and enthusiasts maintaining and improving software in their own time, independent of any institutional mandate. This
volunteer tradition is not incidental to FOSS; it is one of its defining strengths, and NMHS staff engaging with FOSS
as individuals (for example, fixing a bug in a package they rely on, or maintaining a personal project) should be
encouraged and supported to do so in this same tradition.

Institutions can support this by establishing clear policies on individual contribution in a professional context: for
example, permitting staff to submit fixes upstream, rather than only patching locally, when a defect is found in the
course of their work, clarifying intellectual property and attribution where contributions are made using institutional
time or resources, and recognizing upstream contribution as a legitimate and valuable form of technical work.

==== Organizational management of FOSS activities
...
should consider platform accessibility (for example,connectivity constraints in some regions), governance and data sovereignty when making a choice. Some platforms, notably GitLab (Community Edition),
...
Most major hosting platforms include integrated CI/CD tooling (for example,GitHub Actions, GitLab CI) that enables automated
...
forums, chat platforms (for example: Matrix, Slack, Discourse, etc.), and documentation sites. When evaluating or adopting a FOSS
